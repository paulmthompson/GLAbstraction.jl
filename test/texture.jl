...
using GLAbstraction, FixedSizeArrays, GLWindow, ModernGL, GLFW, Colors, FactCheck

win=create_glcontext("test_texture")
...
facts() do
	@fact toglsltype_string(rg2Df) 	--> "uniform sampler2D"
	@fact toglsltype_string(rgb2Df) --> "uniform sampler2D"
	@fact toglsltype_string(rgba2Df) --> "uniform sampler2D"

	@fact ndims(rgba2Df) --> 2
	@fact eltype(rgba2Df) --> Vec4
end
